# Chapter 28 MyBatis入门 - 理论篇

## 一、学习定位

本章主题是 **MyBatis入门**，核心内容包括：Mapper、XML/注解、动态 SQL、结果映射。它在 Java 后端路线中的作用不是单独记 API，而是把前面学过的 Java 基础逐步落到真实后端项目里。

- 优先级：L1 必须掌握
- 预计投入：4小时
- 阶段产出：用 MyBatis 实现博客 CRUD

## 二、核心概念

### 1. Mapper 接口

理解 Mapper 接口 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 2. SQL 映射

理解 SQL 映射 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 3. 动态 SQL

理解 动态 SQL 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 4. 结果映射

理解 结果映射 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 5. 事务边界

理解 事务边界 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。
...
- 完成“用 MyBatis 实现博客 CRUD”，形成可运行、可演示、可复盘的阶段成果。
...
1. MyBatis入门 解决了 Java 后端项目里的什么问题？
2. 如果本章能力在生产环境出问题，你会从哪些日志、配置或数据开始排查？
3. 本章技术和前后章节的关系是什么？例如它如何服务于博客 API 项目？
4. 你在实现“用 MyBatis 实现博客 CRUD”时会如何划分模块，为什么？
