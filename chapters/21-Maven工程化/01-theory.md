# Chapter 21 Maven工程化 - 理论篇

## 一、学习定位

本章主题是 **Maven工程化**，核心内容包括：pom、依赖、生命周期、插件、多模块概念。它在 Java 后端路线中的作用不是单独记 API，而是把前面学过的 Java 基础逐步落到真实后端项目里。

- 优先级：L1 必须掌握
- 预计投入：3小时
- 阶段产出：把 mini-utils 改造成 Maven 工程

## 二、核心概念

### 1. 标准目录结构

理解 标准目录结构 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 2. 依赖管理

理解 依赖管理 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 3. 生命周期

理解 生命周期 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 4. 插件机制

理解 插件机制 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。

### 5. 多模块边界

理解 多模块边界 时要同时关注三个问题：它解决什么项目问题、代码里如何落地、面试时如何讲清楚取舍。
...
- 完成“把 mini-utils 改造成 Maven 工程”，形成可运行、可演示、可复盘的阶段成果。
...
1. Maven工程化 解决了 Java 后端项目里的什么问题？
2. 如果本章能力在生产环境出问题，你会从哪些日志、配置或数据开始排查？
3. 本章技术和前后章节的关系是什么？例如它如何服务于博客 API 项目？
4. 你在实现“把 mini-utils 改造成 Maven 工程”时会如何划分模块，为什么？
