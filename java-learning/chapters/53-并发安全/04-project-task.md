...
本章项目任务是：**完成并发计数器实验**。任务必须服务于最终目标：能独立交付 Spring Boot REST API，并能在面试中讲清设计和实现。

## 业务背景

博客系统需要逐步具备数据访问、接口设计、认证授权、缓存、安全、测试、部署和面试包装能力。本章负责补齐其中的 **并发安全** 能力，不能只停留在知识点阅读。

## 任务拆解

1. 阅读本章理论文档，提取 synchronized、Lock、Atomic、volatile、可见性 的关键点。
2. 实现或设计“完成并发计数器实验”的最小闭环。
