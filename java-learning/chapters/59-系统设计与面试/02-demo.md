# Chapter 59 系统设计与面试 - 实操 Demo

## Demo 目标

完成一个围绕 **完成 3 道系统设计题笔记** 的最小可运行练习。Demo 不追求功能多，而是要求能运行、能解释、能扩展。
...
1. 创建本章练习分支或目录，名称包含 chapter-59。
2. 根据“限流、缓存、排行榜、短链、登录系统设计”列出 3 个必须验证的行为。
3. 先写最小代码让主流程跑通，再补充异常、边界和日志。
4. 把运行命令、输入样例、输出结果写进本章笔记。
5. 最后用一句话总结：系统设计与面试 在博客项目中承担什么责任。

## 示例代码

~~~md
项目一句话：基于 Spring Boot + MyBatis + Redis 的博客后端系统，覆盖认证授权、文章管理、缓存优化、接口文档和 Docker 部署。
技术亮点：统一异常、参数校验、JWT 登录、RBAC 权限、Redis 缓存、MockMvc 测试、Docker Compose 一键启动。
~~~
...
~~~bash
git add .
git commit -m "chapter 59: 系统设计与面试 demo"
~~~
